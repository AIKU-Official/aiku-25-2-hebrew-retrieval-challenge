...
def predict(query, preprocessed_data):
    ...
    try:
        ...
        # Fallback to KnessetE5-only retrieval with KnessetE5 scores
        try:
            query_embedding = retriever.embed_texts([query_text], is_query=True, batch_size=1)
            KnessetE5_scores = cosine_similarity(query_embedding, retriever.corpus_embeddings)[0]
            top_indices = np.argsort(KnessetE5_scores)[::-1][:20]
            
            results = []
            for idx in top_indices:
                results.append({
                    'paragraph_uuid': retriever.corpus_ids[idx],
                    'score': float(KnessetE5_scores[idx])  # Use actual KnessetE5 cosine similarity score
                })
