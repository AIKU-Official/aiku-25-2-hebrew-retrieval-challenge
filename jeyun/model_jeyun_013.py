...
import numpy as np
import os
from transformers import AutoTokenizer, AutoModel, AutoModelForSequenceClassification, AutoModelForSeq2SeqLM # Added AutoModelForSeq2SeqLM
...
class Translator:
    def __init__(self, model_name=None, device=None):
        """
        Initializes the MT5 translator model.
        """
        if model_name is None:
            local_model_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'models', 'mt5-small')
            if os.path.isdir(local_model_path):
                model_name = local_model_path
                print(f"Using local MT5 model from: {model_name}")
            else:
                # Fallback to downloading if local not found, or specify exact HF path
                model_name = "mt5-small"
                print(f"Local MT5 model not found, will attempt to download from Hugging Face: {model_name}")
        ...
        print(f"Loading MT5 translator on device: {self.device}")
        self.tokenizer = AutoTokenizer.from_pretrained(model_name)
        # Use AutoModelForSeq2SeqLM for T5/MT5 models
    ...
    def translate(self, texts, src_lang="he", tgt_lang="en", batch_size=16, max_new_tokens=256):
        """
        Translates a list of texts from source to target language using MT5.
        """
        translated_texts = []
        # T5/MT5 models often benefit from explicit task prefixes
        # For Hebrew to English, "translate Hebrew to English: " is a common format.
        # You might need to experiment with this prefix.
        prompts = [f"translate {src_lang} to {tgt_lang}: {text.strip()}" for text in texts]

        total_batches = (len(prompts) + batch_size - 1) // batch_size
        for i in tqdm(range(0, len(prompts), batch_size), desc=f"Translating {src_lang.upper()} to {tgt_lang.upper()}"):
            ...
            try:
                inputs = self.tokenizer(
                    ...
                    padding=True,
                    truncation=True,
                    max_length=512, # Max input length for the prompt + source text
                    return_tensors='pt'
                ).to(self.device)

                with torch.no_grad():
                    # Generate translation. You might tune num_beams for quality/speed.
                    outputs = self.model.generate(
                        **inputs,
                        max_new_tokens=max_new_tokens,
                        num_beams=2,  # Using 2 beams for a balance of speed and quality
                        early_stopping=True
                    )
                ...
                print(f"CUDA OOM during translation at batch {i//batch_size + 1}, reducing batch size for remaining items...")
                # If OOM, try processing one by one for the rest of this batch
                    ...
                    try:
                        ...
                        with torch.no_grad():
                            outputs = self.model.generate(
                                **inputs,
                                max_new_tokens=max_new_tokens,
                                num_beams=1, # No beams for single item to conserve memory
                                early_stopping=True
                            )
                        translated_texts.append(self.tokenizer.decode(outputs[0], skip_special_tokens=True))
                        del inputs, outputs
                        if torch.cuda.is_available():
                            torch.cuda.empty_cache()
                ...
                translated_texts.extend([""] * len(batch_prompts)) # Append empty strings for failed batch
...
def preprocess(corpus_dict, data_dir: str):
    """
    Preprocessing function using MT5 translation, then E5 multilingual model + BGE reranker.
    ...
    print("=" * 60)
    print("PREPROCESSING: Initializing MT5 Translator + E5 + BGE Reranker Pipeline...")
    print("=" * 60)
    start_time = time.time() 
    os.environ['PYTORCH_CUDA_ALLOC_CONF'] = 'expandable_segments:True'
    
    # Initialize MT5 translator
    print("Loading MT5 Translator...")
    translator = Translator(model_name=os.path.join(os.path.dirname(os.path.abspath(__file__)), 'models', 'mt5-small'))
    ...
    # --- #[추가됨] Request 2: 영어로 번역된 코퍼스 저장 ---
    output_file_path = os.path.join(data_dir, "english_corpus_013.jsonl")
    print(f"\nSaving translated corpus to {output_file_path}...")
    try:
        with open(output_file_path, 'w', encoding='utf-8') as f:
            for doc_id, translated_text in corpus_translated_texts.items():
                # 원본과 유사한 JSONL 형식으로 저장
                json_line = json.dumps({"uuid": doc_id, "translated_passage": translated_text}, ensure_ascii=False)
                f.write(json_line + '\n')
        print(f"✓ Successfully saved {len(corpus_translated_texts)} translated documents.")
    except Exception as e:
        print(f"[Warning] Failed to save translated corpus file: {e}", file=sys.stderr)
    # --- #[추가됨] End ---
    ...
    retriever.corpus_embeddings = retriever.embed_texts(translated_passages, is_query=False, batch_size=128)
...
def predict(query, preprocessed_data):
    """
    Two-stage prediction with MT5 translation: E5 retrieval + BGE reranking.
