...
class E5LargeRetriever:
    ...
    def embed_texts(self, texts, is_query=False, batch_size=16, layer_idx=-1):
        """
        Generates embeddings using E5 model, optionally from an intermediate transformer layer.
        layer_idx: which hidden layer to use (-1 = last layer)
        """
        if is_query:
        ...
        for i in range(0, len(prefixed_texts), batch_size):
            ...
            try:
                ...
                with torch.no_grad():
                    ...
                    model_output = self.model(**encoded, output_hidden_states=True, return_dict=True)
                    hidden_states = model_output.hidden_states
                    selected_hidden = hidden_states[layer_idx]  # e.g., -1=last, 18=18th layer

                    attention_mask = encoded['attention_mask']

                    # Mean pooling with mask
                    mask_expanded = attention_mask.unsqueeze(-1).expand(selected_hidden.size()).float()
                    sum_embeddings = torch.sum(selected_hidden * mask_expanded, 1)
                ...
                print(f"CUDA OOM at batch {batch_num}, retrying with smaller batch")
                # fallback 처리 생략 가능
...
def preprocess(corpus_dict):
    ...
    retriever.corpus_embeddings = retriever.embed_texts(passages, is_query=False, batch_size=16, layer_idx=-12)  # Use 18th layer embeddings
