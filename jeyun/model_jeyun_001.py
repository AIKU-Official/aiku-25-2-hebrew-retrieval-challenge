...
"""
DictaBERT + BGE Reranker Pipeline for Hebrew Text Retrieval
...
class dictabertRetriever:
      def __init__(self, model_name=None, device=None):
         """
         Initializes the DictaBERT retriever for Hebrew text.
         """
         SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__)) # 현재 파일의 디렉토리
         BASE_DIR = os.path.dirname(os.path.dirname(os.path.dirname(SCRIPT_DIR))) # 상위 폴더 세 칸이동 시 비로소 /app/project_code 도달.


         local_model_path = os.path.join(BASE_DIR, 'Dataset and Baseline/baseline_submission/models', 'dictabert')
            
         # 2. 로컬 경로 확인 및 모델 이름 설정
         if os.path.isdir(local_model_path):
               model_name = local_model_path
               print(f"Using local DictaBERT model from: {model_name}")
         else:
               # 🚨 로컬 경로가 없으면 HuggingFace ID를 할당합니다. (수정된 부분)
               model_name = "dicta-il/dictabert" # <--- DictaBERT의 Hub ID (예시)
               print(f"Local model not found. Falling back to Hub model: {model_name}")
         
         # 3. 최종적으로 model_name이 None이 아닌지 확인 (보안용)
         if model_name is None:
               raise ValueError("Model name is missing after local/remote checks.")
         ...
         print(f"Loading DictaBERT model on device: {self.device}")
      ...
      def embed_texts(self, texts, is_query=False, batch_size=32):
         """
         Generates embeddings for texts using DictaBERT model.
         """
         all_embeddings = []
             ...
             try:
                 ...
                 for single_text in batch_texts:
                     try:
                         ...
                         # DictaBERT-base has 768 dimensions
...
class BGEReranker:
    def __init__(self, model_name=None, device=None):
        ...
        local_model_path = os.path.join(BASE_DIR, 'Dataset and Baseline/baseline_submission/models', 'bge-reranker-v2-m3')
        model_name = local_model_path
