...
try:
    from model_jeyun_013 import preprocess, predict
    print(f"Successfully imported preprocess and predict from model_jeyun_013.py in {MODEL_DIR}")
except ImportError as e:
    print(f"Error: Failed to import model_jeyun_013: {e}", file=sys.stderr)
    print(f"Check if '{MODEL_DIR}/model_jeyun_013.py' exists and is named correctly.", file=sys.stderr)
    sys.exit(1)
# --- 임포트 설정 끝 ---
...
def run_evaluation(data_dir: str, output_path: str):
    ...
    try:
        ...
        preprocessed_data = preprocess(corpus, data_dir)
    ...
    for i, query in enumerate(queries):
        ...
        # 결과 요약 출력
        print(f"Query {i}/{len(queries)} completed in {result_entry['time_ms']:.2f}ms. | NDCG@20: {ndcg_20:.4f}")
    ...
    try:
        ...
        output_filename = os.path.join(output_path, f"test_jeyun_013_{timestamp}.json")
