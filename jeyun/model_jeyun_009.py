...
def predict(query, preprocessed_data):
    ...
    # 3. 결과 통합 (UUID 기준으로 중복 제거 후 점수 합산/병합 로직 추가 필요)
    # 여기서는 간단히 두 결과를 합치고 중복 제거
    candidate_map = {res['paragraph_uuid']: res for res in bm25_results}
    for res in e5_results:
        candidate_map[res['paragraph_uuid']] = res # Dense 결과를 Sparse 결과에 덮어씌움 (간단한 병합)

    candidate_ids = list(candidate_map.keys())
    candidate_passages = [candidate_map[doc_id]['text'] for doc_id in candidate_ids]
