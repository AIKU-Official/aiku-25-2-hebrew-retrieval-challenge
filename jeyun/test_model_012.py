...
try:
    from model_jeyun_012 import preprocess, predict
    print(f"Successfully imported preprocess and predict from model_jeyun_012.py in {MODEL_DIR}")
except ImportError as e:
    print(f"Error: Failed to import model_jeyun_012: {e}", file=sys.stderr)
    print(f"Check if '{MODEL_DIR}/model_jeyun_012.py' exists and is named correctly.", file=sys.stderr)
    sys.exit(1)
# --- 임포트 설정 끝 ---
...
def run_evaluation(data_dir: str, output_path: str):
    ...
    try:
        ...
        output_filename = os.path.join(output_path, f"test_jeyun_012_{timestamp}.json")
