...
"""
- Class/func names, docstrings, print logs == your reference
- Internally uses AlephBERT-Gimmel bi-encoder as the retriever
- If BGE reranker folder exists -> 2-stage; else retriever-only fallback
"""
...
ALEPH_DIR = os.path.join(_THIS_DIR, 'models', 'outputs_dicta_ndcg2026', 'best_biencoder')  # <- AlephBERT-Gimmel
BGE_DIR   = os.path.join(_THIS_DIR, 'models', 'bge-reranker-v2-m3')
...
# =========================
# Retriever (name kept as E5Retriever for compatibility)
# =========================
class E5Retriever:
    def __init__(self, model_name=None, device=None):
        ...
        """
        # Use local model  (actually AlephBERT-Gimmel bi-encoder)
        if model_name is None:
            local_model_path = ALEPH_DIR
            if os.path.isdir(local_model_path):
                model_name = local_model_path
                print(f"Using local E5 model from: {model_name}")
            else:
                raise FileNotFoundError(
                    f"[ERROR] AlephBERT-Gimmel retriever not found.\n"
                    f"Expected local directory: {local_model_path}"
                )
        ...
        # local_files_only to avoid net calls
        self.tokenizer = AutoTokenizer.from_pretrained(model_name, local_files_only=True)
        self.model = AutoModel.from_pretrained(model_name, torch_dtype=_dtype(), local_files_only=True).to(self.device)
        self.model.eval()
    ...
    def embed_texts(self, texts, is_query=False, batch_size=32):
        """
        Generates embeddings for texts using AlephBERT-Gimmel (mean pooling + L2).
        NOTE: No 'query:'/ 'passage:' prefixes (they’re for E5; AlephBERT doesn’t need them).
        """
        if not texts:
        ...
        for i in range(0, len(texts), batch_size):
            ...
            batch_texts = [(t or "").strip() for t in texts[i:i + batch_size]]
            ...
            try:
                ...
                # Clear GPU memory
                del encoded, model_output, embeddings, last_hidden, attention_mask, mask_expanded, sum_embeddings, sum_mask
                if torch.cuda.is_available():
                    torch.cuda.empty_cache()
                ...
                for single_text in batch_texts:
                    try:
                        ...
                        del encoded, output, last, attn, mask, summed, count, emb
                        if torch.cuda.is_available():
                            torch.cuda.empty_cache()

                    except Exception as e2:
                        print(f"Failed to process single text: {e2}")
                        zero_embedding = torch.zeros(1, self.hidden_size).float()
...
class BGEReranker:
    def __init__(self, model_name=None, device=None):
        ...
        # Use local model
        if model_name is None:
            local_model_path = BGE_DIR
            if os.path.isdir(local_model_path):
                model_name = local_model_path
                print(f"Using local BGE model from: {model_name}")
            else:
                # Mark as unavailable; retriever-only path will be used
                self.available = False
                self.device = device or _device()
                print(f"[INFO] BGE reranker not found at {local_model_path}. Will use retriever-only.")
                return
...
def preprocess(corpus_dict):
    ...
    # Initialize retriever (AlephBERT-Gimmel under the hood)
    print("Loading E5 retriever...")
    retriever = E5Retriever()

    # Initialize BGE reranker (optional)
    ...
    retriever.corpus_embeddings = retriever.embed_texts(passages, is_query=False, batch_size=EMB_BATCH)
...
def predict(query, preprocessed_data):
    ...
    try:
        ...
        top_100_indices = np.argsort(e5_scores)[::-1][:TOPK_CAND]
        ...
        # If BGE is unavailable, rerank() returns []; fall back to retriever-only top-20
        if not reranked_results:
            top_indices = np.argsort(e5_scores)[::-1][:TOPK_RETURN]
            return [
                {
